    ...
    raw_node root = y_combinator{[&](auto&& convert) -> raw_node {
        if (index >= flat_schema.size()) {
            throw parquet_exception::corrupted_file(seastar::format(
              "Could not build schema tree: unexpected end of flat schema {} {}", index, flat_schema.size()));
        }
...
primitive_node build_primitive_node(const raw_node& r) {
    try {
        return primitive_node{
          {r.info, r.path, r.def_level, r.rep_level}, logical_type::read_logical_type(r.info), r.column_index};
    } catch (const std::exception& e) {
        // throw parquet_exception(seastar::format("Error while processing schema node {}: {}",
        // r.path.front(), e.what()));
        throw parquet_exception(seastar::format("Error while processing schema node: {}", e.what()));
    }
}
...
list_node build_list_node(const raw_node& r) {
    ...
    if ((repeated_node.children.size() != 1) || (repeated_node.info.name == "array") ||
        (repeated_node.info.name == (r.info.name + "_tuple"))) {
        // Legacy 2-level list
        return list_node{{r.info, r.path, r.def_level, r.rep_level},
...
map_node build_map_node(const raw_node& r) {
    ...
    return map_node{{r.info, r.path, r.def_level, r.rep_level},
...
node build_logical_node(const raw_node& r) {
    ...
    if (r.info.repetition_type == format::FieldRepetitionType::OPTIONAL) {
        return optional_node{{r.info, r.path, r.def_level - 1, r.rep_level},
                             std::make_unique<node>(build_unwrapped_node())};
    } else if (r.info.repetition_type == format::FieldRepetitionType::REPEATED) {
        return list_node{{r.info, r.path, r.def_level - 1, r.rep_level - 1},
                         std::make_unique<node>(build_unwrapped_node())};
    } else {
...
void compute_leaves(schema& root) {
    auto collect = y_combinator{[&](auto&& collect, const node& x_variant) -> void {
        std::visit(overloaded{[&](const optional_node& x) { collect(*x.child); },
                              ...
                              [&](const primitive_node& y) { root.leaves.push_back(&y); }},
                   x_variant);
    }};
