...
logical_type read_logical_type(const format::SchemaElement& x) {
    static auto verify = [](bool condition, const std::string& error) {
        if (!condition) {
            throw parquet_exception::corrupted_file(error);
        }
    };
    if (!x.__isset.type) {
        return UNKNOWN{};
...
void write_logical_type(logical_type logical_type, format::SchemaElement& leaf) {
    ...
    return std::visit(overloaded{[&](const STRING&) {
                                 ...
                                 [&](const INTERVAL&) { leaf.__set_converted_type(format::ConvertedType::INTERVAL); },
                                 [&](const JSON&) {
